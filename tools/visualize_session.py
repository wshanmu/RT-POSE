...
Output PNGs can be assembled into a video with ffmpeg:
    ffmpeg -framerate 10 -i /tmp/viz_session13/%05d.png -c:v libx264 out.mp4
"""
...
# ---------------------------------------------------------------------------
# COCO-17 body + COCO-WholeBody feet  (the 23 keypoints used in training)
# Joint order:
#   0-16 : standard COCO-17 body keypoints
#   17-19: left foot  (LBigToe, LSmallToe, LHeel)
#   20-22: right foot (RBigToe, RSmallToe, RHeel)
# ---------------------------------------------------------------------------
JOINT_NAMES = [
    "Nose",       # 0
    "LEye",       # 1
...
SKELETON_EDGES = [
    ...
    # right arm
    (6, 8), (8, 10),
    # torso (shoulder bar, hip bar, shoulder-hip laterals)
    (5, 6), (5, 11), (6, 12), (11, 12),
    # left leg
    (11, 13), (13, 15),
    # right leg
    (12, 14), (14, 16),
    # left foot: ankle → big toe → small toe, ankle → heel
    (15, 17), (17, 18), (15, 19),
    # right foot: ankle → big toe → small toe, ankle → heel
    (16, 20), (20, 21), (16, 22),
]
...
def _edge_color(i, j):
    ...
    right_joints = {2, 4, 6, 8, 10, 12, 14, 16, 20, 21, 22}
...
# ---------------------------------------------------------------------------
ROI = {"x": (1.0, 3.0), "y": (-1.25, 1.25), "z": (-1.15, 1.40)}   # depth, lateral, height
...
def _mpjpe_cm(pred_kps, gt_pose):
    ...
    if not pred_kps or gt_pose is None:
        return None
    gt = np.array(gt_pose)          # (N, 3)
    n_gt = gt.shape[0]
    pred_by_id = {int(kp[0]): kp[1:4] for kp in pred_kps}
    errors = []
    for jid in range(n_gt):
        if jid in pred_by_id:
            errors.append(np.linalg.norm(np.array(pred_by_id[jid]) - gt[jid]))
    if not errors:
        return None
...
def make_skeleton_frame(pred_kps, gt_pose, frame_id, mpjpe_cm_val):
    ...
    def _kps_to_xyz(kps):
        if not kps:
            return None
        n = max(k[0] for k in kps) + 1
        arr_kp = np.zeros((n, 3))
        for k in kps:
            arr_kp[k[0]] = [k[1], k[2], k[3]]
        return arr_kp
    ...
    # elev, azim, title
    # azim=-90 → camera along +X, shows Y-Z (front/face-on)
    # azim=0   → camera along -Y, shows X-Z (left-side view)
    views = [
...
def make_frame(arr, pred_kps, gt_pose, frame_id, mpjpe_cm_val):
...
def parse_args():
    ...
    group.add_argument("--session-dir", help="Session folder containing DZYX_npy_f16/ and Train.json")
    ...
    parser.add_argument("--skeleton-only", action="store_true",
        help="Save a compact 2-panel skeleton-only image (side + front) instead of the full radar figure")
            ...
            if label_path.exists():
                ...
                for seq_key, seq_frames in gt_data.items():
                    for fid, objs in seq_frames.items():
                        ...
                        gt_lookup[fid] = pose if len(pose) == 23 else None
